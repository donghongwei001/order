	...
	public String gouwuchegengxin(){
		...
		try {
			...
			for (int i = 0; i < list2.size(); i++) {
				...
				int flag1=new DaoFactory().executeUpdate("insert into order_food(fk_order_id,fk_food_id,order_food_num,order_food_status) values(20,?,?,1)", params3);
			...
			QueryRunner qr = new QueryRunner(cp.getDataSource());
			List<OverOrderBean> tableList=qr.query("select food_name, food_price,order_food_num from food_table,order_food where food_table.food_id=order_food.fk_food_id and order_food.fk_order_id=20", new BeanListHandler<OverOrderBean>(OverOrderBean.class));
