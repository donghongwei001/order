...
public class NaturalPersonDao {
		public List<OrderBean> queryPerson(int startIndex,int maxPage){
			...
			String sql="select top "+maxPage+" o.order_id,o.order_time,o.order_fk_tabid,o.order_money,e.emp_name,o.order_dt_score,o.oeder_dt_mark,t.code_name from code_table t, order_table o,emp_table e where t.code_id=o.order_status and o.order_fk_empid=e.emp_id and order_id not in(select top "+startIndex+" order_id from order_table order by order_id desc)order by order_id desc";
			...
			try {
				...
				while(rs.next()){
					...
					o.setOrder_dt_score(rs.getInt(6));
					o.setOeder_dt_mark(rs.getString(7));
					o.setCode_name(rs.getString(8));
					ListOrder.add(o);
				}
				ps.close();
				rs.close();
